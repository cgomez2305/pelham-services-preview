...
- **Target keyword placement** (title/H1/copy, one primary phrase per page, nothing forced): "California to Florida freight shipping" (Services pillar), "transloading services" + facility breadth (Transloading), "dedicated freight carrier California" (Dedicated Transport), "LTL vs FTL consolidation California Florida" (Freight Consolidation), "trucking company Gardena CA" / "trucking company Miami FL" (each terminal page — see [seo-strategy.md](seo-strategy.md) for why the Miami terminal page targets "trucking company" rather than "transloading" even though that terminal is also a transloading hub).
...
- **Industry use cases and service comparisons**: `/services/dedicated-transport/` covers retail, manufacturing and temperature-sensitive freight; `/services/transloading/` includes a "transloading vs. direct transport" decision block; `/services/freight-consolidation/` covers LTL vs. FTL — all per the client's content brief.
- **Image formats**: no WebP/AVIF conversion was needed because the site has no photographic raster imagery to begin with — every visual is inline SVG except the small brand logo/favicon PNGs (a few KB each). That's a deliberate consequence of the "no stock photography" rule, not an oversight; if real fleet/warehouse photography is added later, it should ship as WebP with a JPEG fallback.
- **Core Web Vitals**: not yet measured with a live Lighthouse/PageSpeed run (no invented numbers) — but the site is structurally built for it: no render-blocking scripts, no large images, `font-display: swap` on Google Fonts, and reserved SVG `viewBox` dimensions to avoid layout shift. Run a real audit once deployed to the final domain and update this section with actual figures.
